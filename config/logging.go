...
type LogSrcListener func(old, new slog.LogTargetCfg)
...
// Get fetches the current, cached configuration
func (src *jsonSrc) Get() slog.LogTargetCfg {
	src.mutex.RLock()
...
func (src *jsonSrc) set(cfg slog.LogTargetCfg) {
	src.mutex.Lock()
...
// Get fetches the current, cached configuration
func (src *fileSrc) Get() slog.LogTargetCfg {
	src.mutex.RLock()
	defer src.mutex.RUnlock()
	return src.cfg
}
...
func (src *fileSrc) Set(path string, fget FileGetter) error {
	...
	watcher, err := newWatcher(path, func() {
		if serr := src.Set(path, fget); serr != nil {
			slog.Error("Failed to reload file on change", slog.String("path", path), slog.Err(serr))
		}
	})
	if err != nil {
		return err
...
func (src *fileSrc) set(cfg slog.LogTargetCfg) {
	src.mutex.Lock()
	defer src.mutex.Unlock()
