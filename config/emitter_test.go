...
func TestLogSrcEmitter(t *testing.T) {
	...
	expectedOldCfg := make(slog.LogTargetCfg)
	expectedNewCfg := make(slog.LogTargetCfg)

	listener1 := false
	id1 := e.AddListener(func(oldCfg, newCfg slog.LogTargetCfg) {
		assert.Equal(t, expectedOldCfg, oldCfg)
		assert.Equal(t, expectedNewCfg, newCfg)
		listener1 = true
	})
	...
	listener2 := false
	id2 := e.AddListener(func(oldCfg, newCfg slog.LogTargetCfg) {
		assert.Equal(t, expectedOldCfg, oldCfg)
		assert.Equal(t, expectedNewCfg, newCfg)
		listener2 = true
	})
