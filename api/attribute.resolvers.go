...
func (r *Resolver) AttributeCreate(ctx context.Context, args struct{ Input AttributeCreateInput }) (*AttributeCreate, error) {
	...
	if inputType != nil && *inputType == model.AttributeInputTypeReference &&
		...
		return nil, model_helper.NewAppError("AttributeCreate", "api.attribute.entity_type_missing.app_error", nil, "entity type is required when REFERENCE input type is used", http.StatusBadRequest)
	}
...
func (r *Resolver) AttributeUpdate(ctx context.Context, args struct {
	...
	if len(args.Input.RemoveValues) > 0 {
		...
		if !lo.EveryBy(removeValues, func(vl *model.AttributeValue) bool { return vl.AttributeID == attribute.Id }) {
			return nil, model_helper.NewAppError("AttributeUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "removeValues"}, "one of attribute values does not belong to given attribute", http.StatusBadRequest)
		}
...
func (r *Resolver) AttributeValueCreate(ctx context.Context, args struct {
	...
	if attribute.InputType == model.AttributeInputTypeSwatch {
		...
		if (fileUrl != nil && *fileUrl != "") ||
			...
			return nil, model_helper.NewAppError("AttributeValueCreate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "fileUrl or contentType"}, "fieUrl and contentType can only be defined for swatch attribute", http.StatusBadRequest)
		}
	}
	...
	attrValue := &model.AttributeValue{
		Name:        args.Input.Name,
		RichText:    model_types.JSONString(args.Input.RichText),
		FileUrl:     args.Input.FileURL,
		ContentType: args.Input.ContentType,
...
func (r *Resolver) AttributeValueDelete(ctx context.Context, args struct{ Id UUID }) (*AttributeValueDelete, error) {
	...
	if len(attrValues) == 0 {
		return nil, model_helper.NewAppError("AttributeValueDelete", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "Id"}, "id="+args.Id.String()+" is in valid", http.StatusBadRequest)
	}
...
func (r *Resolver) AttributeValueUpdate(ctx context.Context, args struct {
	...
	if len(attrValues) == 0 {
		return nil, model_helper.NewAppError("AttributeValueUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "Id"}, "id="+args.Id.String()+" is in valid", http.StatusBadRequest)
	}
	...
	attrValue.RichText = model_types.JSONString(args.Input.RichText)
...
func (r *Resolver) AttributeReorderValues(ctx context.Context, args AttributeReorderValuesArgs) (*AttributeReorderValues, error) {
	...
	for _, move := range args.Moves {
		if !attributeValueMap[move.ID] { // not contains
			return nil, model_helper.NewAppError("AttributeReorderValues", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "move.Id"}, fmt.Sprintf("attribute value with id=%s does not belong to the attribute", move.ID), http.StatusBadRequest)
		}
...
func (args *AttributesArgs) parse() (*model.AttributeFilterOption, *model_helper.AppError) {
	// validate params
	...
	if args.ChannelSlug != nil {
		if !slug.IsSlug(*args.ChannelSlug) {
			return nil, model_helper.NewAppError("AttributesArgs.parse", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "channel slug"}, "please provide valid channel slug", http.StatusBadRequest)
		}
		attributeFilter.ChannelSlug = args.ChannelSlug
	}

	if args.SortBy != nil && !args.SortBy.Field.IsValid() {
		return nil, model_helper.NewAppError("AttributesArgs.parse", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "sort field"}, "please provide valid sort field", http.StatusBadRequest)
	}
...
func (r *Resolver) Attributes(ctx context.Context, args AttributesArgs) (*AttributeCountableConnection, error) {
	...
	if err != nil {
		return nil, model_helper.NewAppError("Attributes", "app.attribute.count_by_options.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) Attribute(ctx context.Context, args struct {
	...
	if (args.Id == nil && args.Slug == nil) || (args.Id != nil && args.Slug != nil) {
		return nil, model_helper.NewAppError("Attribute", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id/slug"}, "please provide id or slug, not both", http.StatusBadRequest)
	}
	if args.Slug != nil && !slug.IsSlug(*args.Slug) {
		return nil, model_helper.NewAppError("Attribute", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "slug"}, "please provide valid slug", http.StatusBadRequest)
	}
