...
func (r *Resolver) ShippingMethodChannelListingUpdate(ctx context.Context, args struct {
	...
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid shipping method channel listing id", http.StatusBadRequest)
	}

	if !lo.EveryBy(args.Input.RemoveChannels, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "remove channels"}, "please provide valid remove channel ids", http.StatusBadRequest)
	}
	...
	if !lo.EveryBy(channelIdsRequesterWantToAdd, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "add channels"}, "please provide valid add channel ids", http.StatusBadRequest)
	}
	...
	if len(channelIDsNotAssignedToShippingMethod) > 0 {
		return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "add channels"}, "please provide channels that are assigned to the shipping method", http.StatusBadRequest)
	}
	...
	for _, channelInput := range args.Input.AddChannels {
		if channelInput.Price == nil && !actualChannelIdsCanAddMap[channelInput.ChannelID] {
			return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "add channel"}, "price field is required", http.StatusBadRequest)
		}
		...
		if channelInput.MinimumOrderPrice != nil &&
			...
			return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "add channels"}, "max prices must be greater than min prices", http.StatusBadRequest)
		}
	}
	...
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	...
	if err != nil {
		return nil, model_helper.NewAppError("ShippingMethodChannelListingUpdate", model_helper.ErrorCommittingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) ShippingPriceCreate(ctx context.Context, args struct{ Input ShippingPriceInput }) (*ShippingPriceCreate, error) {
	...
	transaction := embedCtx.App.Srv().Store.GetMaster().Begin()
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("ShippingPriceCreate", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	defer embedCtx.App.Srv().Store.FinalizeTransaction(transaction)
	...
	err := transaction.Commit().Error
	if err != nil {
		return nil, model_helper.NewAppError("ShippingPriceCreate", model_helper.ErrorCreatingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) ShippingPriceDelete(ctx context.Context, args struct{ Id string }) (*ShippingPriceDelete, error) {
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingPriceDelete", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid id", http.StatusBadRequest)
	}
...
func (r *Resolver) ShippingPriceBulkDelete(ctx context.Context, args struct{ Ids []string }) (*ShippingPriceBulkDelete, error) {
	...
	if !lo.EveryBy(args.Ids, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingPriceBulkDelete", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "ids"}, "please provide valid ids", http.StatusBadRequest)
	}
...
func (r *Resolver) ShippingPriceUpdate(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingPriceUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid id", http.StatusBadRequest)
	}
	...
	transaction := embedCtx.App.Srv().Store.GetMaster().Begin()
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("ShippingPriceUpdate", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	defer embedCtx.App.Srv().Store.FinalizeTransaction(transaction)
	...
	err := transaction.Commit().Error
	if err != nil {
		return nil, model_helper.NewAppError("ShippingPriceUpdate", model_helper.ErrorCreatingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) ShippingPriceExcludeProducts(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingPriceExcludeProducts", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid shipping method id", http.StatusBadRequest)
	}

	if !lo.EveryBy(args.Input.Products, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingPriceExcludeProducts", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "product ids"}, "please provide valid product ids", http.StatusBadRequest)
	}
...
func (r *Resolver) ShippingPriceRemoveProductFromExclude(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingPriceRemoveProductFromExclude", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid shipping method id", http.StatusBadRequest)
	}

	if !lo.EveryBy(args.Products, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingPriceRemoveProductFromExclude", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "product ids"}, "please provide valid product ids", http.StatusBadRequest)
	}
...
func (r *Resolver) ShippingZoneCreate(ctx context.Context, args struct {
	...
	// validate params
	if !lo.EveryBy(args.Input.AddWarehouses, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingZoneCreate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "add warehouses"}, "please provide valid warehouse ids", http.StatusBadRequest)
	}
	if !lo.EveryBy(args.Input.AddChannels, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingZoneCreate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "add channels"}, "please provide valid channel ids", http.StatusBadRequest)
	}
	...
	transaction := embedCtx.App.Srv().Store.GetMaster().Begin()
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("ShippingZoneCreate", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	defer embedCtx.App.Srv().Store.FinalizeTransaction(transaction)
	...
	err := transaction.Commit().Error
	if err != nil {
		return nil, model_helper.NewAppError("ShippingZoneCreate", model_helper.ErrorCommittingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) ShippingZoneDelete(ctx context.Context, args struct{ Id string }) (*ShippingZoneDelete, error) {
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingZoneDelete", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid shipping zone id", http.StatusBadRequest)
	}
...
func (r *Resolver) ShippingZoneBulkDelete(ctx context.Context, args struct{ Ids []string }) (*ShippingZoneBulkDelete, error) {
	// validate params
	if !lo.EveryBy(args.Ids, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("ShippingZoneCreate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "ids"}, "please provide valid shipping zone ids", http.StatusBadRequest)
	}
...
func (r *Resolver) ShippingZoneUpdate(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingZoneUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid shipping zone id", http.StatusBadRequest)
	}
	...
	for argsName, ids := range map[string][]string{
		...
		if !lo.EveryBy(ids, model_helper.IsValidId) {
			return nil, model_helper.NewAppError("ShippingZoneUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": argsName}, "please provide valid "+argsName, http.StatusBadRequest)
		}
	}

	if len(lo.Intersect(args.Input.AddWarehouses, args.Input.RemoveWarehouses)) > 0 {
		return nil, model_helper.NewAppError("ShippingZoneUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "AddWarehouses / RemoveWarehouses"}, "add warehouses must differ from remove warehouses", http.StatusBadRequest)
	}
	if len(lo.Intersect(args.Input.AddChannels, args.Input.RemoveChannels)) > 0 {
		return nil, model_helper.NewAppError("ShippingZoneUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "AddChannels / RemoveChannels"}, "add channels must differ from remove channels", http.StatusBadRequest)
	}
	...
	transaction := embedCtx.App.Srv().Store.GetMaster().Begin()
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("ShippingZoneUpdate", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	defer embedCtx.App.Srv().Store.FinalizeTransaction(transaction)
	...
	err := transaction.Commit().Error
	if err != nil {
		return nil, model_helper.NewAppError("ShippingZoneUpdate", model_helper.ErrorCommittingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) ShippingZone(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("ShippingZone", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid shipping zone id", http.StatusBadRequest)
	}
...
func (r *Resolver) ShippingZones(ctx context.Context, args struct {
	...
	if args.ChannelID != nil {
		if !model_helper.IsValidId(*args.ChannelID) {
			return nil, model_helper.NewAppError("ShippingZones", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "channel id"}, "please provide valid channel id", http.StatusBadRequest)
		}
	...
	if args.Filter != nil {
		if !lo.EveryBy(args.Filter.Channels, model_helper.IsValidId) {
			return nil, model_helper.NewAppError("ShippingZones", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "channels"}, "please provide valid channel id", http.StatusBadRequest)
		}
