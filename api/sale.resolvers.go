...
func (r *Resolver) SaleCreate(ctx context.Context, args struct{ Input SaleInput }) (*SaleCreate, error) {
	...
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("Resolver.SaleCreate", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	...
	if err != nil {
		return nil, model_helper.NewAppError("SaleCreate", "app.discount.create_sale.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
	...
	if err != nil {
		return nil, model_helper.NewAppError("SaleCreate", model_helper.ErrorCommittingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) SaleDelete(ctx context.Context, args struct{ Id string }) (*SaleDelete, error) {
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("SaleDelete", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid sale id", http.StatusBadRequest)
	}
	...
	transaction := embedCtx.App.Srv().Store.GetMaster().Begin()
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("Resolver.SaleDelete", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	defer embedCtx.App.Srv().Store.FinalizeTransaction(transaction)
	...
	if err != nil {
		return nil, model_helper.NewAppError("SaleDelete", "app.discount.delete_sale_by_id.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) SaleBulkDelete(ctx context.Context, args struct{ Ids []string }) (*SaleBulkDelete, error) {
	...
	if !lo.EveryBy(args.Ids, model_helper.IsValidId) {
		return nil, model_helper.NewAppError("SaleBulkDelete", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "ids"}, "please provide valid sale ids", http.StatusBadRequest)
	}
	...
	if err != nil {
		return nil, model_helper.NewAppError("SaleBulkDelete", "app.discount.delete_sale_by_id.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) SaleCataloguesAdd(ctx context.Context, args struct {
	...
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("SaleCataloguesAdd", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "Id"}, "please provide valid sale id", http.StatusBadRequest)
	}
	...
	if len(args.Input.Products) > 0 {
		...
		if len(productsWithNovariants) > 0 {
			return nil, model_helper.NewAppError("SaleCataloguesAdd", "app.discount.add_products_with_no_variants_to_sale.app_error", nil, "cant add products that have no variants to sale", http.StatusNotAcceptable)
		}
	}
	...
	transaction := embedCtx.App.Srv().Store.GetMaster().Begin()
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("Resolver.SaleCataloguesAdd", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	defer embedCtx.App.Srv().Store.FinalizeTransaction(transaction)
	...
	err := transaction.Commit().Error
	if err != nil {
		return nil, model_helper.NewAppError("SaleCataloguesAdd", model_helper.ErrorCommittingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) SaleCataloguesRemove(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("SaleCataloguesRemove", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid sale id", http.StatusBadRequest)
	}
	...
	transaction := embedCtx.App.Srv().Store.GetMaster().Begin()
	if transaction.Error != nil {
		return nil, model_helper.NewAppError("SaleCataloguesRemove", model_helper.ErrorCreatingTransactionErrorID, nil, transaction.Error.Error(), http.StatusInternalServerError)
	}
	defer embedCtx.App.Srv().Store.FinalizeTransaction(transaction)
	...
	err := transaction.Commit().Error
	if err != nil {
		return nil, model_helper.NewAppError("SaleCataloguesRemove", model_helper.ErrorCommittingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) SaleChannelListingUpdate(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("SaleChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "id"}, "please provide valid sale id", http.StatusBadRequest)
	}
	...
	for _, addChannelObj := range args.Input.AddChannels {
		...
		case model.DISCOUNT_VALUE_TYPE_PERCENTAGE:
			if decimal.Decimal(addChannelObj.DiscountValue).GreaterThan(decimal.NewFromInt(100)) {
				// discount can't > 100%
				return nil, model_helper.NewAppError("SaleChannelListingUpdate", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "add channels"}, "discount cannot be greater than 100%", http.StatusBadRequest)
			}
		}
	}
	...
	if tran.Error != nil {
		return nil, model_helper.NewAppError("SaleChannelListingUpdate", model_helper.ErrorCreatingTransactionErrorID, nil, tran.Error.Error(), http.StatusInternalServerError)
	}
	...
	if err != nil {
		return nil, model_helper.NewAppError("SaleChannelListingUpdate", "app.sale.error_delete_sale_channel_listings.app_error", nil, err.Error(), http.StatusInternalServerError)
	}
	...
	if err != nil {
		...
		return nil, model_helper.NewAppError("SaleChannelListingUpdate", "app.sale.add_sale_channel_listings.app_error", nil, err.Error(), statusCode)
	}
	...
	if err != nil {
		return nil, model_helper.NewAppError("SaleChannelListingUpdate", model_helper.ErrorCommittingTransactionErrorID, nil, err.Error(), http.StatusInternalServerError)
	}
...
func (r *Resolver) Sale(ctx context.Context, args struct {
	...
	// validate params
	if !model_helper.IsValidId(args.Id) {
		return nil, model_helper.NewAppError("Resolve.Sale", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "Id"}, "please provide valid sale id", http.StatusBadRequest)
	}
...
func (a *SalesArgs) parse() (*model.SaleFilterOption, *model_helper.AppError) {
	...
	if a.SortBy != nil && !a.SortBy.Field.IsValid() {
		return nil, model_helper.NewAppError("SalesArgs.SortBy", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "Field"}, "please provide valid field to sort", http.StatusBadRequest)
	}
	if a.Channel != nil && !slug.IsSlug(*a.Channel) {
		return nil, model_helper.NewAppError("SalesArgs.Channel", model_helper.InvalidArgumentAppErrorID, map[string]any{"Fields": "channel"}, *a.Channel+" is not a valid channel slug", http.StatusBadRequest)
	}
