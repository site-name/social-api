...
import (
	...
	request "github.com/sitename/sitename/app/request"
)
...
func (_m *SamlInterface) CheckProviderAttributes(SS *model.SamlSettings, ouser *account.User, patch *account.UserPatch) string {
	ret := _m.Called(SS, ouser, patch)

	var r0 string
	if rf, ok := ret.Get(0).(func(*model.SamlSettings, *account.User, *account.UserPatch) string); ok {
...
func (_m *SamlInterface) DoLogin(c *request.Context, encodedXML string, relayState map[string]string) (*account.User, *model.AppError) {
	ret := _m.Called(c, encodedXML, relayState)

	var r0 *account.User
	if rf, ok := ret.Get(0).(func(*request.Context, string, map[string]string) *account.User); ok {
