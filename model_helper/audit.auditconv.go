...
func newAuditUser(u *model.User) auditUser {
	var user auditUser
	if u != nil {
		user.ID = u.ID
...
func newAuditJob(j *model.Job) auditJob {
	var job auditJob
	if j != nil {
		job.ID = j.ID
		job.Type = j.Type.String()
...
func newAuditRole(r *model.Role) auditRole {
	var role auditRole
	if r != nil {
		role.ID = r.ID
		role.Name = r.Name
		role.DisplayName = r.DisplayName
		role.Permissions = strings.Fields(r.Permissions)
...
func newAuditSession(s *model.Session) auditSession {
