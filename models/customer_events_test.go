...
func testCustomerEventToOneOrderUsingOrderidOrder(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOrderidOrder(ctx, tx, false, (*[]*CustomerEvent)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCustomerEventToOneUserUsingUseridUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CustomerEventSlice{&local}
	if err = local.L.LoadUseridUser(ctx, tx, false, (*[]*CustomerEvent)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCustomerEventToOneSetOpOrderUsingOrderidOrder(t *testing.T) {
	var err error
	...
	for i, x := range []*Order{&b, &c} {
		...
		if x.R.OrderidCustomerEvents[0] != &a {
...
func testCustomerEventToOneRemoveOpOrderUsingOrderidOrder(t *testing.T) {
	var err error
	...
	if len(b.R.OrderidCustomerEvents) != 0 {
		t.Error("failed to remove a from b's relationships")
	}
}

func testCustomerEventToOneSetOpUserUsingUseridUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		...
		if x.R.UseridCustomerEvents[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
...
func testCustomerEventToOneRemoveOpUserUsingUseridUser(t *testing.T) {
	var err error
	...
	if len(b.R.UseridCustomerEvents) != 0 {
		t.Error("failed to remove a from b's relationships")
	}
}
