...
func (productVariantTranslationL) LoadProductVariant(e boil.Executor, singular bool, maybeProductVariantTranslation any, mods queries.Applicator) error {
...
func (o *ProductVariantTranslation) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(productVariantTranslationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(productVariantTranslationPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert product_variant_translations, could not build conflict column list")
			}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"product_variant_translations\"", updateOnConflict, ret, update, conflict, insert, opts...)
