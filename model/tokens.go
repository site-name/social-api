...
type Token struct {
	...
	Extra     string `boil:"extra" json:"extra" toml:"extra" yaml:"extra"`
...
var TokenWhere = struct {
	...
	Extra:     whereHelperstring{field: "\"tokens\".\"extra\""},
}
...
var (
	...
	tokenColumnsWithoutDefault = []string{"token", "created_at", "type", "extra"}
...
func (o *Token) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(tokenPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert tokens, could not build conflict column list")
			}
...
func (o TokenSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"tokens\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, tokenPrimaryKeyColumns, len(o))
