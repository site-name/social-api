...
func (o *Fulfillment) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(fulfillmentAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(fulfillmentPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert fulfillments, could not build conflict column list")
			}

			conflict = make([]string, len(fulfillmentPrimaryKeyColumns))
			copy(conflict, fulfillmentPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"fulfillments\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o FulfillmentSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"fulfillments\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, fulfillmentPrimaryKeyColumns, len(o))
