...
func (saleTranslationL) LoadSale(e boil.Executor, singular bool, maybeSaleTranslation any, mods queries.Applicator) error {
...
func (o *SaleTranslation) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(saleTranslationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(saleTranslationPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert sale_translations, could not build conflict column list")
			}

			conflict = make([]string, len(saleTranslationPrimaryKeyColumns))
			copy(conflict, saleTranslationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"sale_translations\"", updateOnConflict, ret, update, conflict, insert, opts...)
