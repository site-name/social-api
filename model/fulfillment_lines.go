...
func (r *fulfillmentLineR) GetGiftcards() GiftcardSlice {
	if r == nil {
		return nil
	}
	return r.Giftcards
...
func (o *FulfillmentLine) Giftcards(mods ...qm.QueryMod) giftcardQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"giftcards\".\"fulfillment_line_id\"=?", o.ID),
	)
...
func (fulfillmentLineL) LoadGiftcards(e boil.Executor, singular bool, maybeFulfillmentLine interface{}, mods queries.Applicator) error {
	var slice []*FulfillmentLine
	var object *FulfillmentLine
	...
	query := NewQuery(
		qm.From(`giftcards`),
		qm.WhereIn(`giftcards.fulfillment_line_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		for _, foreign := range resultSlice {
			...
			foreign.R.FulfillmentLine = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.FulfillmentLineID) {
				local.R.Giftcards = append(local.R.Giftcards, foreign)
				if foreign.R == nil {
					foreign.R = &giftcardR{}
				}
				foreign.R.FulfillmentLine = local
				break
			}
		}
	}
...
// of the fulfillment_line, optionally inserting them as new records.
// Appends related to o.R.Giftcards.
// Sets related.R.FulfillmentLine appropriately.
func (o *FulfillmentLine) AddGiftcards(exec boil.Executor, insert bool, related ...*Giftcard) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.FulfillmentLineID, o.ID)
...
// fulfillment_line replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.FulfillmentLine's Giftcards accordingly.
// Replaces o.R.Giftcards with related.
// Sets related.R.FulfillmentLine's Giftcards accordingly.
func (o *FulfillmentLine) SetGiftcards(exec boil.Executor, insert bool, related ...*Giftcard) error {
	query := "update \"giftcards\" set \"fulfillment_line_id\" = null where \"fulfillment_line_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.Giftcards {
			queries.SetScanner(&rel.FulfillmentLineID, nil)
			if rel.R == nil {
				continue
			}

			rel.R.FulfillmentLine = nil
		}
		o.R.Giftcards = nil
	}
...
// Sets related.R.FulfillmentLine.
func (o *FulfillmentLine) RemoveGiftcards(exec boil.Executor, related ...*Giftcard) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(exec, boil.Whitelist("fulfillment_line_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
...
func (o FulfillmentLineSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"fulfillment_lines\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, fulfillmentLinePrimaryKeyColumns, len(o))
