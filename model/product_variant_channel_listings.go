...
var ProductVariantChannelListingTableColumns = struct {
	...
	DiscountedPriceAmount:     "product_variant_channel_listings.discounted_price_amount",
...
var ProductVariantChannelListingWhere = struct {
	...
	DiscountedPriceAmount:     whereHelpermodel_types_NullDecimal{field: "\"product_variant_channel_listings\".\"discounted_price_amount\""},
...
var ProductVariantChannelListingRels = struct {
	Channel                                                  string
	Variant                                                  string
	VariantChannelListingVariantChannelListingPromotionRules string
}{
	Channel: "Channel",
	Variant: "Variant",
	VariantChannelListingVariantChannelListingPromotionRules: "VariantChannelListingVariantChannelListingPromotionRules",
}
...
type productVariantChannelListingR struct {
	...
	VariantChannelListingVariantChannelListingPromotionRules VariantChannelListingPromotionRuleSlice `boil:"VariantChannelListingVariantChannelListingPromotionRules" json:"VariantChannelListingVariantChannelListingPromotionRules" toml:"VariantChannelListingVariantChannelListingPromotionRules" yaml:"VariantChannelListingVariantChannelListingPromotionRules"`
}
...
func (r *productVariantChannelListingR) GetVariantChannelListingVariantChannelListingPromotionRules() VariantChannelListingPromotionRuleSlice {
	if r == nil {
		return nil
	}
	return r.VariantChannelListingVariantChannelListingPromotionRules
}
...
var (
	productVariantChannelListingAllColumns            = []string{"id", "variant_id", "channel_id", "currency", "price_amount", "cost_price_amount", "preorder_quantity_threshold", "created_at", "discounted_price_amount", "annotations"}
	productVariantChannelListingColumnsWithoutDefault = []string{"id", "variant_id", "channel_id", "created_at"}
	productVariantChannelListingColumnsWithDefault    = []string{"currency", "price_amount", "cost_price_amount", "preorder_quantity_threshold", "discounted_price_amount", "annotations"}
...
// VariantChannelListingVariantChannelListingPromotionRules retrieves all the variant_channel_listing_promotion_rule's VariantChannelListingPromotionRules with an executor via variant_channel_listing_id column.
func (o *ProductVariantChannelListing) VariantChannelListingVariantChannelListingPromotionRules(mods ...qm.QueryMod) variantChannelListingPromotionRuleQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"variant_channel_listing_promotion_rules\".\"variant_channel_listing_id\"=?", o.ID),
	)
...
// LoadVariantChannelListingVariantChannelListingPromotionRules allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (productVariantChannelListingL) LoadVariantChannelListingVariantChannelListingPromotionRules(e boil.Executor, singular bool, maybeProductVariantChannelListing interface{}, mods queries.Applicator) error {
	var slice []*ProductVariantChannelListing
	var object *ProductVariantChannelListing
	...
	query := NewQuery(
		...
		qm.WhereIn(`variant_channel_listing_promotion_rules.variant_channel_listing_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.VariantChannelListingVariantChannelListingPromotionRules = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &variantChannelListingPromotionRuleR{}
			}
			foreign.R.VariantChannelListing = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.VariantChannelListingID {
				local.R.VariantChannelListingVariantChannelListingPromotionRules = append(local.R.VariantChannelListingVariantChannelListingPromotionRules, foreign)
				if foreign.R == nil {
					foreign.R = &variantChannelListingPromotionRuleR{}
				}
				foreign.R.VariantChannelListing = local
				break
			}
		}
	}
...
// AddVariantChannelListingVariantChannelListingPromotionRules adds the given related objects to the existing relationships
// of the product_variant_channel_listing, optionally inserting them as new records.
// Appends related to o.R.VariantChannelListingVariantChannelListingPromotionRules.
// Sets related.R.VariantChannelListing appropriately.
func (o *ProductVariantChannelListing) AddVariantChannelListingVariantChannelListingPromotionRules(exec boil.Executor, insert bool, related ...*VariantChannelListingPromotionRule) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.VariantChannelListingID = o.ID
	...
	if o.R == nil {
		o.R = &productVariantChannelListingR{
			VariantChannelListingVariantChannelListingPromotionRules: related,
		}
	} else {
		o.R.VariantChannelListingVariantChannelListingPromotionRules = append(o.R.VariantChannelListingVariantChannelListingPromotionRules, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &variantChannelListingPromotionRuleR{
				VariantChannelListing: o,
			}
		} else {
			rel.R.VariantChannelListing = o
		}
	}
	return nil
}
...
func (o ProductVariantChannelListingSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"product_variant_channel_listings\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, productVariantChannelListingPrimaryKeyColumns, len(o))
