...
func (o *PageTranslation) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(pageTranslationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(pageTranslationPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert page_translations, could not build conflict column list")
			}

			conflict = make([]string, len(pageTranslationPrimaryKeyColumns))
			copy(conflict, pageTranslationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"page_translations\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o PageTranslationSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"page_translations\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, pageTranslationPrimaryKeyColumns, len(o))
