...
func (assignedVariantAttributeValueL) LoadAssignment(e boil.Executor, singular bool, maybeAssignedVariantAttributeValue interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`assigned_variant_attributes.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *AssignedVariantAttributeValue) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(assignedVariantAttributeValueAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(assignedVariantAttributeValuePrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert assigned_variant_attribute_values, could not build conflict column list")
			}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"assigned_variant_attribute_values\"", updateOnConflict, ret, update, conflict, insert, opts...)
