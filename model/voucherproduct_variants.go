	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(voucherproductVariantPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert voucherproduct_variants, could not build conflict column list")
			}

			conflict = make([]string, len(voucherproductVariantPrimaryKeyColumns))
			copy(conflict, voucherproductVariantPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"voucherproduct_variants\"", updateOnConflict, ret, update, conflict, insert, opts...)
