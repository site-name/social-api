...
var (
	...
	attributeTranslationColumnsWithoutDefault = []string{"id", "attribute_id", "language_code", "name"}
	attributeTranslationColumnsWithDefault    = []string{}
...
func (o *AttributeTranslation) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(attributeTranslationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(attributeTranslationPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert attribute_translations, could not build conflict column list")
			}

			conflict = make([]string, len(attributeTranslationPrimaryKeyColumns))
			copy(conflict, attributeTranslationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"attribute_translations\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o AttributeTranslationSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"attribute_translations\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, attributeTranslationPrimaryKeyColumns, len(o))
