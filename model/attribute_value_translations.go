...
func (o *AttributeValueTranslation) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(attributeValueTranslationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(attributeValueTranslationPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert attribute_value_translations, could not build conflict column list")
			}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"attribute_value_translations\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o AttributeValueTranslationSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"attribute_value_translations\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, attributeValueTranslationPrimaryKeyColumns, len(o))
