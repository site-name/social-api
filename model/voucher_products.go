...
func (o *VoucherProduct) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(voucherProductAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(voucherProductPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert voucher_products, could not build conflict column list")
			}

			conflict = make([]string, len(voucherProductPrimaryKeyColumns))
			copy(conflict, voucherProductPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"voucher_products\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o VoucherProductSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"voucher_products\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, voucherProductPrimaryKeyColumns, len(o))
