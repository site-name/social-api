...
func (o *ProductMedium) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(productMediumAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(productMediumPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert product_media, could not build conflict column list")
			}

			conflict = make([]string, len(productMediumPrimaryKeyColumns))
			copy(conflict, productMediumPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"product_media\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o ProductMediumSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"product_media\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, productMediumPrimaryKeyColumns, len(o))
