...
func (scheme *Scheme) ToJson() string {
	b, _ := json.JSON.Marshal(scheme)
	return string(b)
}

func SchemeFromJson(data io.Reader) *Scheme {
	var scheme *Scheme
	json.JSON.NewDecoder(data).Decode(&scheme)
	return scheme
}

func SchemesToJson(schemes []*Scheme) string {
	b, _ := json.JSON.Marshal(schemes)
	return string(b)
}

func SchemesFromJson(data io.Reader) []*Scheme {
	var schemes []*Scheme
	if err := json.JSON.NewDecoder(data).Decode(&schemes); err == nil {
		return schemes
	}
	return nil
}
