...
func (attributePageL) LoadPageType(e boil.Executor, singular bool, maybeAttributePage interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`page_types`),
		qm.WhereIn(`page_types.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *AttributePage) SetPageType(exec boil.Executor, insert bool, related *PageType) error {
	var err error
	if insert {
...
func (o AttributePageSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"attribute_pages\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, attributePagePrimaryKeyColumns, len(o))
