...
var (
	...
	roleColumnsWithoutDefault = []string{"id", "name", "display_name", "description", "created_at", "updated_at", "permissions", "scheme_managed", "built_in"}
	roleColumnsWithDefault    = []string{"delete_at"}
...
func (o *Role) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(rolePrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert roles, could not build conflict column list")
			}
...
func (o RoleSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"roles\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, rolePrimaryKeyColumns, len(o))
