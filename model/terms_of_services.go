...
func (q termsOfServiceQuery) One(exec boil.Executor) (*TermsOfService, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: failed to execute a one query for terms_of_services")
	}
...
func FindTermsOfService(exec boil.Executor, iD string, selectCols ...string) (*TermsOfService, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: unable to select from terms_of_services")
	}
...
func (o *TermsOfService) Insert(exec boil.Executor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to insert into terms_of_services")
	}
...
func (o *TermsOfService) Update(exec boil.Executor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("model: unable to update terms_of_services, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by update for terms_of_services")
	}
...
func (q termsOfServiceQuery) UpdateAll(exec boil.Executor, cols M) (int64, error) {
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected for terms_of_services")
	}
...
func (o TermsOfServiceSlice) UpdateAll(exec boil.Executor, cols M) (int64, error) {
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected all in update all termsOfService")
	}
	return rowsAff, nil
}
...
func (o *TermsOfService) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("model: unable to upsert terms_of_services, could not build update column list")
		}

		ret := strmangle.SetComplement(termsOfServiceAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(termsOfServicePrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert terms_of_services, could not build conflict column list")
			}

			conflict = make([]string, len(termsOfServicePrimaryKeyColumns))
			copy(conflict, termsOfServicePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"terms_of_services\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *TermsOfService) Delete(exec boil.Executor) (int64, error) {
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by delete for terms_of_services")
	}
...
func (q termsOfServiceQuery) DeleteAll(exec boil.Executor) (int64, error) {
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by deleteall for terms_of_services")
	}
...
func (o TermsOfServiceSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"terms_of_services\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, termsOfServicePrimaryKeyColumns, len(o))
...
func TermsOfServiceExists(exec boil.Executor, iD string) (bool, error) {
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "model: unable to check if terms_of_services exists")
	}
