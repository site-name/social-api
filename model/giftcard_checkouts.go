...
func (o *GiftcardCheckout) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(giftcardCheckoutAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(giftcardCheckoutPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert giftcard_checkouts, could not build conflict column list")
			}

			conflict = make([]string, len(giftcardCheckoutPrimaryKeyColumns))
			copy(conflict, giftcardCheckoutPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"giftcard_checkouts\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o GiftcardCheckoutSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"giftcard_checkouts\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, giftcardCheckoutPrimaryKeyColumns, len(o))
