...
type VoucherCustomer struct {
	...
	VoucherCodeID string `boil:"voucher_code_id" json:"voucher_code_id" toml:"voucher_code_id" yaml:"voucher_code_id"`
...
var VoucherCustomerTableColumns = struct {
	...
	VoucherCodeID: "voucher_customers.voucher_code_id",
...
var VoucherCustomerWhere = struct {
	...
	VoucherCodeID whereHelperstring
	CustomerEmail whereHelperstring
}{
	ID:            whereHelperstring{field: "\"voucher_customers\".\"id\""},
	VoucherCodeID: whereHelperstring{field: "\"voucher_customers\".\"voucher_code_id\""},
...
type voucherCustomerR struct {
	VoucherCode *VoucherCode `boil:"VoucherCode" json:"VoucherCode" toml:"VoucherCode" yaml:"VoucherCode"`
}
...
func (r *voucherCustomerR) GetVoucherCode() *VoucherCode {
	if r == nil {
		return nil
	}
	return r.VoucherCode
}
...
var (
	voucherCustomerAllColumns            = []string{"id", "voucher_code_id", "customer_email"}
	voucherCustomerColumnsWithoutDefault = []string{"id", "voucher_code_id", "customer_email"}
...
// VoucherCode pointed to by the foreign key.
func (o *VoucherCustomer) VoucherCode(mods ...qm.QueryMod) voucherCodeQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"id\" = ?", o.VoucherCodeID),
	}
...
// LoadVoucherCode allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (voucherCustomerL) LoadVoucherCode(e boil.Executor, singular bool, maybeVoucherCustomer interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args[object.VoucherCodeID] = struct{}{}
	...
	query := NewQuery(
		...
		qm.WhereIn(`voucher_codes.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load VoucherCode")
	}

	var resultSlice []*VoucherCode
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice VoucherCode")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for voucher_codes")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.VoucherCodeID == foreign.ID {
				local.R.VoucherCode = foreign
				if foreign.R == nil {
					foreign.R = &voucherCodeR{}
				}
...
// SetVoucherCode of the voucherCustomer to the related item.
// Sets o.R.VoucherCode to related.
// Adds o to related.R.VoucherCustomers.
func (o *VoucherCustomer) SetVoucherCode(exec boil.Executor, insert bool, related *VoucherCode) error {
	var err error
	if insert {
	...
	o.VoucherCodeID = related.ID
	if o.R == nil {
		o.R = &voucherCustomerR{
			VoucherCode: related,
		}
	} else {
		o.R.VoucherCode = related
	}

	if related.R == nil {
		related.R = &voucherCodeR{
...
func (o VoucherCustomerSlice) DeleteAll(exec boil.Executor) (int64, error) {
	...
	sql := "DELETE FROM \"voucher_customers\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, voucherCustomerPrimaryKeyColumns, len(o))
