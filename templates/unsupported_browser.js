...
document.addEventListener('DOMContentLoaded', function () {
    var hovers = document.querySelectorAll("div[data-sitename-hover]");
    for (var i = 0; i < hovers.length; i++) {
        var element = hovers[i];
        element.addEventListener("mouseover", function(e) {
            onHover(e.currentTarget, e.currentTarget.getAttribute("data-sitename-hover"));
        });
        element.addEventListener("mouseout", function(e) {
            onUnHover(e.currentTarget, e.currentTarget.getAttribute("data-sitename-hover"))
        });
    }
    var clicks = document.querySelectorAll("div[data-sitename-click], button[data-sitename-click]");
    for (var i = 0; i < clicks.length; i++) {
        var element = clicks[i];
        element.addEventListener("click", function(e) {
            window.open(e.currentTarget.getAttribute('data-sitename-click'), '_blank');
        });
    };
});