...
func (_m *StatusStore) CreateIndexesIfNotExists() {
	_m.Called()
}

// Get provides a mock function with given fields: userID
func (_m *StatusStore) Get(userID string) (*account.Status, error) {
	ret := _m.Called(userID)

	var r0 *account.Status
	if rf, ok := ret.Get(0).(func(string) *account.Status); ok {
		r0 = rf(userID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*account.Status)
		}
	}
...
func (_m *StatusStore) GetByIds(userIds []string) ([]*account.Status, error) {
...
func (_m *StatusStore) SaveOrUpdate(status *account.Status) error {
	ret := _m.Called(status)

	var r0 error
	if rf, ok := ret.Get(0).(func(*account.Status) error); ok {
		r0 = rf(status)
	} else {
		r0 = ret.Error(0)
	}
