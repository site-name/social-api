...
func (ps *SqlCategoryStore) CreateIndexesIfNotExists() {
	ps.CreateIndexIfNotExists("idx_categories_name", store.ProductCategoryTableName, "Name")
	ps.CreateIndexIfNotExists("idx_categories_slug", store.ProductCategoryTableName, "Slug")
	ps.CreateIndexIfNotExists("idx_categories_name_lower_textpattern", store.ProductCategoryTableName, "lower(Name) text_pattern_ops")
}
