...
CREATE TYPE customer_event_type AS ENUM (
	 ...
	 'EMAIL_CHANGED',
	 'PLACED_ORDER',            -- created an order
	 'NOTE_ADDED_TO_ORDER',     -- added a note to one of their orders
	 'DIGITAL_LINK_DOWNLOADED', -- downloaded a digital good
	 'CUSTOMER_DELETED',        -- staff user deleted a customer
	 'EMAIL_ASSIGNED',          -- the staff user assigned a email to the customer
	 'NAME_ASSIGNED',           -- the staff user added set a name to the customer
	 'NOTE_ADDED',              -- the staff user added a note to the customer
	 'ACCOUNT_ACTIVATED',
	 'ACCOUNT_DEACTIVATED'
);
END IF;
...
DO $$
BEGIN
	IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname ILIKE 'Currency')
THEN
