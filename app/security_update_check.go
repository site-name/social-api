...
func (s *Server) DoSecurityUpdateCheck() {
	...
	if currentTime-lastSecurityTime > SecurityUpdatePeriod {
		slog.Debug("Checking for security update from Sitename")
	}
	...
	systemSecurityLastTime := &model.System{
		Name:  model.SYSTEM_LAST_SECURITY_TIME,
		Value: strconv.FormatInt(currentTime, 10),
	}
	...
	// unt, err := s.Store.Team().AnalyticsTeamCount(false); err == nil {
	// 	v.Set(PropSecurityTeamCount, strconv.FormatInt(teamCount, 10))
	// }

	res, err := http.Get(PropSecurityURL + "/security?" + v.Encode())
	if err != nil {
		slog.Error("failed to get security update information from Sitename.")
		return
	}
	...
	for _, bulletin := range bulletins {
		if bulletin.AppliesToVersion == model.CurrentVersion {
			if props["SecurityBulletin_"+bulletin.Id] == "" {
				...
				if userErr != nil {
					slog.Error("Failed to get system admins for security update information from Sitename")
					return
				}
