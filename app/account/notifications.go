...
func (s *ServiceAccount) SendPasswordResetNotification(redirectURL string, user account.User, manager interfaces.PluginManagerInterface, channelID string) *model.AppError {
	var (
	...
	_, appErr := manager.Notify(plugins.ACCOUNT_PASSWORD_RESET, payload, channelID, "")
	return appErr
}

// Trigger sending an account confirmation notification for the given user
func (s *ServiceAccount) SendAccountConfirmation(redirectUrl string, user account.User, manager interfaces.PluginManagerInterface, channelID string) *model.AppError {
	var (
		token           = util.DefaultTokenGenerator.MakeToken(&user)
		params          = url.Values{"email": []string{user.Email}, "token": []string{token}}
	...
	_, appErr := manager.Notify(plugins.ACCOUNT_CONFIRMATION, payload, channelID, "")
	return appErr
}

// Trigger sending a notification change email for the given user
func (s *ServiceAccount) SendRequestUserChangeEmailNotification(redirectUrl string, user account.User, newEmail string, token string, manager interfaces.PluginManagerInterface, channelID string) *model.AppError {
	var (
	...
	_, appErr := manager.Notify(plugins.ACCOUNT_CHANGE_EMAIL_REQUEST, payload, channelID, "")
	return appErr
}

// Trigger sending a email change notification for the given user
func (s *ServiceAccount) SendUserChangeEmailNotification(recipientEmail string, user account.User, manager interfaces.PluginManagerInterface, channelID string) *model.AppError {
	payload := model.StringInterface{
		"user":            s.GetDefaultUserPayload(&user),
	...
	_, appErr := manager.Notify(plugins.ACCOUNT_CHANGE_EMAIL_CONFIRM, payload, channelID, "")
	return appErr
}

// SendAccountDeleteConfirmationNotification Trigger sending a account delete notification for the given user
func (s *ServiceAccount) SendAccountDeleteConfirmationNotification(redirectUrl string, user account.User, manager interfaces.PluginManagerInterface, channelID string) *model.AppError {
	var (
		token          = util.DefaultTokenGenerator.MakeToken(&user)
		params         = url.Values{"token": []string{token}}
	...
	_, appErr := manager.Notify(plugins.ACCOUNT_DELETE, payload, channelID, "")
	return appErr
}

// Trigger sending a set password notification for the given customer/staff.
func (s *ServiceAccount) SendSetPasswordNotification(redirectUrl string, user account.User, manager interfaces.PluginManagerInterface, channelID string) *model.AppError {
	var (
		token               = util.DefaultTokenGenerator.MakeToken(&user)
	...
	_, appErr := manager.Notify(plugins.ACCOUNT_SET_CUSTOMER_PASSWORD, payload, channelID, "")
	return appErr
}
