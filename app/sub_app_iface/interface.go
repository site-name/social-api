...
type CheckoutService interface {
	...
	DeleteCheckoutLines(transaction *gorp.Transaction, checkoutLineIDs []string) *model.AppError                                                                                                                                             // DeleteCheckoutLines deletes all checkout lines by given uuid list
	...
	AddVariantsToCheckout(ckout *checkout.Checkout, variants []*product_and_discount.ProductVariant, quantities []int, channelSlug string, skipStockCheck, replace bool) (*checkout.Checkout, *exception.InsufficientStock, *model.AppError) // AddVariantsToCheckout add variants to checkout
	...
	CheckoutSubTotal(manager interface{}, checkoutInfo *checkout.CheckoutInfo, lines []*checkout.CheckoutLineInfo, address *account.Address, discounts []*product_and_discount.DiscountInfo) (*goprices.TaxedMoney, *model.AppError)         // CheckoutSubTotal Return the total cost of all the checkout lines, taxes included.  It takes in account all plugins.
}
...
type ProductService interface {
	...
	ProductVariantTranslationsByOption(option *product_and_discount.ProductVariantTranslationFilterOption) ([]*product_and_discount.ProductVariantTranslation, *model.AppError)                                         // ProductVariantTranslationsByOption returns a list of product variant translations
	ProductTranslationsByOption(option *product_and_discount.ProductTranslationFilterOption) ([]*product_and_discount.ProductTranslation, *model.AppError)                                                              // ProductTranslationsByOption returns a list of product translations
	ProductVariantChannelListingsByOption(transaction *gorp.Transaction, option *product_and_discount.ProductVariantChannelListingFilterOption) ([]*product_and_discount.ProductVariantChannelListing, *model.AppError) // ProductVariantChannelListingsByOption returns a slice of product variant channel listings by given option
	ProductTypeByOption(options *product_and_discount.ProductTypeFilterOption) (*product_and_discount.ProductType, *model.AppError)                                                                                     // ProductTypeByOption returns a product type with given option
}
...
type WarehouseService interface {
	CheckStockQuantity(variant *product_and_discount.ProductVariant, countryCode string, channelSlug string, quantity int) (*exception.InsufficientStock, *model.AppError)                                                                                                    // Validate if there is stock available for given variant in given country. If so - returns None. If there is less stock then required raise InsufficientStock exception.
	CheckStockQuantityBulk(variants product_and_discount.ProductVariants, countryCode string, quantities []int, channelSlug string, additionalFilterLookup model.StringInterface, existingLines []*checkout.CheckoutLineInfo) (*exception.InsufficientStock, *model.AppError) // Validate if there is stock available for given variants in given country. It raises InsufficientStock: when there is not enough items in stock for a variant
	...
	DecreaseAllocations(lineInfos []*order.OrderLineData, manager interface{}) (*exception.InsufficientStock, *model.AppError)                                                                                                                                                // DecreaseAllocations Decreate allocations for provided order lines.
...
type DiscountService interface {
	...
	ValidateMinSpent(voucher *product_and_discount.Voucher, value *goprices.TaxedMoney, channelID string) (*product_and_discount.NotApplicable, *model.AppError) // ValidateMinSpent validates if the order cost at least a specific amount of money
	ValidateOncePerCustomer(voucher *product_and_discount.Voucher, customerEmail string) (*product_and_discount.NotApplicable, *model.AppError)                  // ValidateOncePerCustomer checks to make sure each customer has ONLY 1 time usage with 1 voucher
	...
	ValidateVoucherOnlyForStaff(voucher *product_and_discount.Voucher, customerID string) (*product_and_discount.NotApplicable, *model.AppError) // ValidateVoucherOnlyForStaff validate if voucher is only for staff
	...
	ValidateVoucherInOrder(ord *order.Order) (*product_and_discount.NotApplicable, *model.AppError)                                                                                                                                                                        // ValidateVoucherInOrder validates order has voucher and the voucher satisfies all requirements
	...
	AddVoucherUsageByCustomer(voucher *product_and_discount.Voucher, customerEmail string) (notApplicableErr *product_and_discount.NotApplicable, appErr *model.AppError)                                                                                                  // AddVoucherUsageByCustomer adds an usage for given voucher, by given customer
	...
	VoucherByOption(options *product_and_discount.VoucherFilterOption) (*product_and_discount.Voucher, *model.AppError)                            // VoucherByOption returns 1 voucher filtered using given options
	VoucherCustomersByOption(options *product_and_discount.VoucherCustomerFilterOption) ([]*product_and_discount.VoucherCustomer, *model.AppError) // VoucherCustomersByOption returns a slice of voucher customers filtered using given options
	VoucherCustomerByOptions(options *product_and_discount.VoucherCustomerFilterOption) (*product_and_discount.VoucherCustomer, *model.AppError)   // VoucherCustomerByOptions finds a voucher customer relation and returns it with an error
}
...
type OrderService interface {
	...
	SendOrderConfirmation(orDer *order.Order, redirectURL string, manager interface{}) *model.AppError                                                                                             // SendOrderConfirmation sends notification with order confirmation
}
