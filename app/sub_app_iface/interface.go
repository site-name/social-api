...
type CheckoutService interface {
	...
	AddVariantsToCheckout(ckout *checkout.Checkout, variants []*product_and_discount.ProductVariant, quantities []int, channelSlug string, skipStockCheck, replace bool) (*checkout.Checkout, *app.InsufficientStock, *model.AppError) // AddVariantsToCheckout add variants to checkout
...
type DiscountService interface {
	...
	ValidateMinSpent(voucher *product_and_discount.Voucher, value *goprices.TaxedMoney, channelID string) (*exception.NotApplicable, *model.AppError) // ValidateMinSpent validates if the order cost at least a specific amount of money
	ValidateOncePerCustomer(voucher *product_and_discount.Voucher, customerEmail string) (*exception.NotApplicable, *model.AppError)                  // ValidateOncePerCustomer checks to make sure each customer has ONLY 1 time usage with 1 voucher
	...
	ValidateVoucherOnlyForStaff(voucher *product_and_discount.Voucher, customerID string) (*exception.NotApplicable, *model.AppError) // ValidateVoucherOnlyForStaff validate if voucher is only for staff
	...
	ValidateVoucherInOrder(ord *order.Order) (*exception.NotApplicable, *model.AppError)                                                                         // ValidateVoucherInOrder validates order has voucher and the voucher satisfies all requirements
	...
	AddVoucherUsageByCustomer(voucher *product_and_discount.Voucher, customerEmail string) (notApplicableErr *exception.NotApplicable, appErr *model.AppError)   // AddVoucherUsageByCustomer adds an usage for given voucher, by given customer
