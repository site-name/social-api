...
type ProductService interface {
	...
	BulkUpsertProductVariantChannelListings(transaction *gorm.DB, listings []*model.ProductVariantChannelListing) ([]*model.ProductVariantChannelListing, *model_helper.AppError)
	// CalculateRevenueForVariant Calculate total revenue generated by a product variant
	//
	// NOTE: `startDate` must be UTC time
	CalculateRevenueForVariant(variant *model.ProductVariant, startDate *time.Time, orderLines []*model.OrderLine, ordersDict map[string]*model.Order, currencyCode string) (*goprices.TaxedMoney, *model_helper.AppError)
	// CategoriesByOption returns all categories that satisfy given option
	CategoriesByOption(option *model.CategoryFilterOption) (model.Categories, *model_helper.AppError)
	// CategoryByOption returns 1 category that satisfies given option
	CategoryByOption(option *model.CategoryFilterOption) (*model.Category, *model_helper.AppError)
	...
	CollectCategoriesTreeProducts(category *model.Category) (model.Products, *model_helper.AppError)
	...
	CollectionProductRelationsByOptions(options *model.CollectionProductFilterOptions) ([]*model.CollectionProduct, *model_helper.AppError)
	// CollectionsByOption returns all collections that satisfy given option.
	//
	// NOTE: `ShopID` is required.
	CollectionsByOption(option *model.CollectionFilterOption) (int64, model.Collections, *model_helper.AppError)
	...
	// GetVariantPrice
	GetVariantPrice(variant model.ProductVariant, variantChannelListing model.ProductVariantChannelListing, product model.Product, collections []*model.Collection, discounts []*model.DiscountInfo, chanNel model.Channel) (*goprices.Money, *model_helper.AppError)
	...
	ProductChannelListingsByOption(option *model.ProductChannelListingFilterOption) ([]*model.ProductChannelListing, *model_helper.AppError)
	...
	ProductTranslationsByOption(option *model.ProductTranslationFilterOption) ([]*model.ProductTranslation, *model_helper.AppError)
	...
	ProductVariantGetPrice(productVariant *model.ProductVariant, product model.Product, collections []*model.Collection, channel model.Channel, channelListing *model.ProductVariantChannelListing, discounts []*model.DiscountInfo) (*goprices.Money, *model_helper.AppError)
	...
	ProductVariantTranslationsByOption(option *model.ProductVariantTranslationFilterOption) ([]*model.ProductVariantTranslation, *model_helper.AppError)
	...
	ProductVariantsByOption(option *model.ProductVariantFilterOption) (model.ProductVariants, *model_helper.AppError)
	// ProductsByOption returns a list of products that satisfy given option
	ProductsByOption(option *model.ProductFilterOption) (model.Products, *model_helper.AppError)
	...
	UpdateProductDiscountedPrice(transaction *gorm.DB, product model.Product, discounts []*model.DiscountInfo) *model_helper.AppError
	// UpdateProductsDiscountedPrices
	UpdateProductsDiscountedPrices(transaction *gorm.DB, products []*model.Product, discounts []*model.DiscountInfo) *model_helper.AppError
	...
	FilterProductsAdvanced(options *model.ExportProductsFilterOptions, channelIdOrSlug string) (model.Products, *model_helper.AppError)
