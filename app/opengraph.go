...
func (a *App) parseOpenGraphMetadata(requestURL string, body io.Reader, contentType string) *opengraph.OpenGraph {
	...
	if err := og.ProcessHTML(body); err != nil {
		slog.Warn("parseOpenGraphMetadata processing failed", slog.String("requestURL", requestURL), slog.Err(err))
	}
...
func forceHTMLEncodingToUTF8(body io.Reader, contentType string) io.Reader {
	...
	if err != nil {
		slog.Warn("forceHTMLEncodingToUTF8 failed to convert", slog.String("contentType", contentType), slog.Err(err))
		return body
	}
...
func makeOpenGraphURLsAbsolute(og *opengraph.OpenGraph, requestURL string) {
	...
	if err != nil {
		slog.Warn("makeOpenGraphURLsAbsolute failed to parse url", slog.String("requestURL", requestURL), slog.Err(err))
		return
	}
	...
	makeURLAbsolute := func(resultURL string) string {
		...
		if err != nil {
			slog.Warn("makeOpenGraphURLsAbsolute failed to parse result", slog.String("requestURL", requestURL), slog.Err(err))
			return resultURL
		}
...
func (a *App) ImageProxyRemover() func(string) string {
	if !*a.Config().ImageProxySettings.Enable {
		return nil
	}
