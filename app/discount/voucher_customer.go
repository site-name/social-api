...
func (a *ServiceDiscount) VoucherCustomerByOptions(options *model.VoucherCustomerFilterOption) (*model.VoucherCustomer, *model.AppError) {
	...
	if err != nil {
		...
		return nil, model.NewAppError("VoucherCustomerByOptions", "app.discount.voucher_customer_missing.app_error", nil, err.Error(), statusCode)
	}
...
func (s *ServiceDiscount) VoucherCustomersByOption(options *model.VoucherCustomerFilterOption) ([]*model.VoucherCustomer, *model.AppError) {
	...
	if err != nil {
		return nil, model.NewAppError("VoucherCustomersByOption", "app.discount.error_finding_voucher_customers_by_options", nil, err.Error(), http.StatusInternalServerError)
	}
