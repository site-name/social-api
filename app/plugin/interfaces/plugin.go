...
type BasePluginInterface interface {
	...
	// Handle authentication request responsible for obtaining access tokens.
	// Overwrite this method if the plugin handles authentication flow.
	ExternalObtainAccessTokens(data model.StringInterface, request *http.Request, previousValue model.ExternalAccessTokens) (*model.ExternalAccessTokens, *model.AppError)
	// Handle authentication refresh request.
	// Overwrite this method if the plugin handles authentication flow and supports
	// refreshing the access.
	ExternalRefresh(data model.StringInterface, request *http.Request, previousValue model.ExternalAccessTokens) (*model.ExternalAccessTokens, *model.AppError)
	...
	ExternalVerify(data model.StringInterface, request *http.Request, previousValue AType) (*model.User, model.StringInterface, *model.AppError)
	// Authenticate user which should be assigned to the request.
	// Overwrite this method if the plugin handles authentication flow.
	AuthenticateUser(request *http.Request, previousValue interface{}) (*model.User, *model.AppError)
	...
	//
	ChangeUserAddress(address model.Address, addressType model.AddressTypeEnum, user *model.User, previousValue model.Address) (*model.Address, *model.AppError)
	...
	CalculateCheckoutTotal(checkoutInfo model.CheckoutInfo, lines model.CheckoutLineInfos, address *model.Address, discounts []*model.DiscountInfo, previousValue goprices.TaxedMoney) (*goprices.TaxedMoney, *model.AppError)
	// Calculate the shipping costs for model.
	// Overwrite this method if you need to apply specific logic for the calculation
	// of shipping costs. Return TaxedMoney.
	CalculateCheckoutShipping(checkoutInfo model.CheckoutInfo, lines model.CheckoutLineInfos, address *model.Address, discounts []*model.DiscountInfo, previousValue goprices.TaxedMoney) (*goprices.TaxedMoney, *model.AppError)
	...
	CalculateOrderShipping(orDer *model.Order, previousValue goprices.TaxedMoney) (*goprices.TaxedMoney, *model.AppError)
	// Calculate checkout line total.
	// Overwrite this method if you need to apply specific logic for the calculation
	// of a checkout line total. Return TaxedMoney.
	CalculateCheckoutLineTotal(checkoutInfo model.CheckoutInfo, lines model.CheckoutLineInfos, checkoutLineInfo model.CheckoutLineInfo, address *model.Address, discounts []*model.DiscountInfo, previousValue goprices.TaxedMoney) (*goprices.TaxedMoney, *model.AppError)
	// Calculate order line total.
	// Overwrite this method if you need to apply specific logic for the calculation
	// of a order line total. Return TaxedMoney.
	CalculateOrderLineTotal(orDer *model.Order, orderLine *model.OrderLine, variant model.ProductVariant, product model.Product, previousValue goprices.TaxedMoney) (*goprices.TaxedMoney, *model.AppError)
	// Calculate checkout line unit price
	CalculateCheckoutLineUnitPrice(checkoutInfo model.CheckoutInfo, lines model.CheckoutLineInfos, checkoutLineInfo model.CheckoutLineInfo, address *model.Address, discounts []*model.DiscountInfo, previousValue goprices.TaxedMoney) (*goprices.TaxedMoney, *model.AppError)
	...
	CalculateOrderLineUnit(orDer model.Order, orderLine model.OrderLine, variant model.ProductVariant, product model.Product, previousValue goprices.TaxedMoney) (*goprices.TaxedMoney, *model.AppError)
	//
	GetCheckoutLineTaxRate(checkoutInfo *model.CheckoutInfo, lines model.CheckoutLineInfos, checkoutLineInfo model.CheckoutLineInfo, address *model.Address, discounts []*model.DiscountInfo, previousValue decimal.Decimal) (*decimal.Decimal, *model.AppError)
	//
	GetOrderLineTaxRate(orDer model.Order, product model.Product, variant model.ProductVariant, address *model.Address, previousValue decimal.Decimal) (*decimal.Decimal, *model.AppError)
	//
	GetCheckoutShippingTaxRate(checkoutInfo model.CheckoutInfo, lines model.CheckoutLineInfos, address *model.Address, discounts []*model.DiscountInfo, previousValue decimal.Decimal) (*decimal.Decimal, *model.AppError)
	//
	GetOrderShippingTaxRate(orDer model.Order, previousValue decimal.Decimal) (*decimal.Decimal, *model.AppError)
	...
	OrderCreated(orDer model.Order, previousValue interface{}) (interface{}, *model.AppError)
	//
	DraftOrderCreated(orDer model.Order, defaultValue interface{}) (interface{}, *model.AppError)
	//
	DraftOrderUpdated(orDer model.Order, defaultValue interface{}) (interface{}, *model.AppError)
	//
	DraftOrderDeleted(orDer model.Order, defaultValue interface{}) (interface{}, *model.AppError)
	// Trigger when order is confirmed by staff.
	// Overwrite this method if you need to trigger specific logic after an order is
	// confirmed.
	OrderConfirmed(orDer model.Order, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when sale is created.
	// Overwrite this method if you need to trigger specific logic after sale is created.
	SaleCreated(sale model.Sale, currentCatalogue model.NodeCatalogueInfo, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when sale is deleted.
	// Overwrite this method if you need to trigger specific logic after sale is deleted.
	SaleDeleted(sale model.Sale, previousCatalogue model.NodeCatalogueInfo, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when sale is updated.
	// Overwrite this method if you need to trigger specific logic after sale is updated.
	SaleUpdated(sale model.Sale, previousCatalogue model.NodeCatalogueInfo, currentCatalogue model.NodeCatalogueInfo, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when invoice creation starts.
	// Overwrite to create invoice with proper data, call invoice.update_invoice.
	InvoiceRequest(orDer model.Order, inVoice model.Invoice, number string, previousValue interface{}) (interface{}, *model.AppError)
	...
	InvoiceDelete(inVoice model.Invoice, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger after invoice is sent.
	InvoiceSent(inVoice model.Invoice, email string, previousValue interface{}) (interface{}, *model.AppError)
	...
	CustomerCreated(customer model.User, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when user is updated.
	// Overwrite this method if you need to trigger specific logic after a user is
	// updated.
	CustomerUpdated(customer model.User, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when product is created.
	// Overwrite this method if you need to trigger specific logic after a product is created.
	ProductCreated(product model.Product, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when product is updated.
	// Overwrite this method if you need to trigger specific logic after a product is updated.
	ProductUpdated(product model.Product, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when product is deleted.
	// Overwrite this method if you need to trigger specific logic after a product is deleted.
	ProductDeleted(product model.Product, variants []int, previousVale interface{}) (interface{}, *model.AppError)
	// Trigger when product variant is created.
	// Overwrite this method if you need to trigger specific logic after a product
	// variant is created.
	ProductVariantCreated(productVariant model.ProductVariant, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when product variant is updated.
	// Overwrite this method if you need to trigger specific logic after a product
	// variant is updated.
	ProductVariantUpdated(variant model.ProductVariant, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when product variant is deleted.
	// Overwrite this method if you need to trigger specific logic after a product
	// variant is deleted.
	ProductVariantDeleted(productVariant model.ProductVariant, previousValue interface{}) (interface{}, *model.AppError)
	// ProductVariantOutOfStock triggered when a product variant is out of stock
	ProductVariantOutOfStock(stock model.Stock, defaultValue interface{}) *model.AppError
	// ProductVariantBackInStock is triggered when a product is available again in stock
	ProductVariantBackInStock(stock model.Stock, defaultValue interface{}) *model.AppError
	// Trigger when order is fully paid.
	// Overwrite this method if you need to trigger specific logic when an order is
	// fully paid.
	OrderFullyPaid(orDer model.Order, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when order is updated.
	// Overwrite this method if you need to trigger specific logic when an order is changed.
	OrderUpdated(orDer model.Order, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when order is cancelled.
	// Overwrite this method if you need to trigger specific logic when an order is
	// canceled.
	OrderCancelled(orDer model.Order, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when order is fulfilled.
	// Overwrite this method if you need to trigger specific logic when an order is fulfilled.
	OrderFulfilled(orDer model.Order, previousValue interface{}) (interface{}, *model.AppError)
	...
	FulfillmentCreated(fulfillment model.Fulfillment, previousValue interface{}) (interface{}, *model.AppError)
	...
	FulfillmentCanceled(fulfillment model.Fulfillment, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when checkout is created.
	// Overwrite this method if you need to trigger specific logic when a checkout is created.
	CheckoutCreated(checkOut model.Checkout, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when checkout is updated.
	// Overwrite this method if you need to trigger specific logic when a checkout is updated.
	CheckoutUpdated(checkOut model.Checkout, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when page is updated.
	// Overwrite this method if you need to trigger specific logic when a page is updated.
	PageUpdated(page_ model.Page, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when page is created.
	// Overwrite this method if you need to trigger specific logic when a page is created.
	PageCreated(page_ model.Page, previousValue interface{}) (interface{}, *model.AppError)
	// Trigger when page is deleted.
	// Overwrite this method if you need to trigger specific logic when a page is deleted.
	PageDeleted(page_ model.Page, previousValue interface{}) (interface{}, *model.AppError)
	// Triggered when ShopFetchTaxRates mutation is called.
	FetchTaxesData(previousValue bool) (bool, *model.AppError)
	//
	InitializePayment(paymentData model.StringInterface, previousValue interface{}) (*model.InitializedPaymentResponse, *model.AppError)
	//
	AuthorizePayment(paymentInformation model.PaymentData, previousValue interface{}) (*model.GatewayResponse, *model.AppError)
	//
	CapturePayment(paymentInformation model.PaymentData, previousValue interface{}) (*model.GatewayResponse, *model.AppError)
	//
	VoidPayment(paymentInformation model.PaymentData, previousValue interface{}) (*model.GatewayResponse, *model.AppError)
	//
	RefundPayment(paymentInformation model.PaymentData, previousValue interface{}) (*model.GatewayResponse, *model.AppError)
	//
	ConfirmPayment(paymentInformation model.PaymentData, previousValue interface{}) (*model.GatewayResponse, *model.AppError)
	//
	ProcessPayment(paymentInformation model.PaymentData, previousValue interface{}) (*model.GatewayResponse, *model.AppError)
	//
	ListPaymentSources(customerID string, previousValue interface{}) ([]*model.CustomerSource, *model.AppError)
	//
	GetClientToken(tokenConfig model.TokenConfig, previousValue interface{}) (string, *model.AppError)
	//
	...
	//
	GetPaymentGateways(currency string, checkOut *model.Checkout, previousValue interface{}) ([]*model.PaymentGateway, *model.AppError)
	//
	...
	ValidatePluginConfiguration(pluginConfiguration *model.PluginConfiguration) *model.AppError
	...
	PreSavePluginConfiguration(pluginConfiguration *model.PluginConfiguration) *model.AppError
	//
	SavePluginConfiguration(pluginConfiguration *model.PluginConfiguration, cleanedData model.StringInterface) (*model.PluginConfiguration, *model.AppError)
