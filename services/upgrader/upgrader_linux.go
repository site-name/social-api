...
func verifySignature(filename string, sigfilename string, publicKey string) error {
	...
	if err != nil {
		slog.Debug("Unable to load the public key to verify the file signature", slog.Err(err))
		return NewInvalidSignature()
	}

	mattermost_tar, err := os.Open(filename)
	if err != nil {
		slog.Debug("Unable to open the Mattermost .tar file to verify the file signature", slog.Err(err))
		return NewInvalidSignature()
	}

	signature, err := os.Open(sigfilename)
	if err != nil {
		slog.Debug("Unable to open the Mattermost .sig file verify the file signature", slog.Err(err))
		return NewInvalidSignature()
	}

	_, err = openpgp.CheckDetachedSignature(keyring, mattermost_tar, signature)
	if err != nil {
		slog.Debug("Unable to verify the Mattermost file signature", slog.Err(err))
		return NewInvalidSignature()
	}
	return nil
}
...
func UpgradeToE0() error {
	if !atomic.CompareAndSwapInt32(&upgrading, 0, 1) {
		slog.Warn("Trying to upgrade while another upgrade is running")
	...
	if err != nil {
		...
		slog.Error("Unable to get the path of the Mattermost executable", slog.Err(err))
		return err
	}
	...
	if err != nil {
		...
		slog.Error("Unable to download the Mattermost server binary file", slog.Int64("percentage", upgradePercentage), slog.String("url", getCurrentVersionTgzUrl()), slog.Err(err))
		upgradePercentage = 0
		return err
	}
	...
	if err != nil {
		...
		slog.Error("Unable to download the signature file of the new Mattermost server", slog.String("url", getCurrentVersionTgzUrl()+".sig"), slog.Err(err))
		upgradePercentage = 0
		return err
	}
	...
	if err != nil {
		...
		slog.Error("Unable to verify the signature of the downloaded file", slog.Err(err))
		return err
	}
	...
	if err != nil {
		upgradePercentage = 0
		upgradeError = err
		slog.Error("Unable to extract the binary from the downloaded file", slog.Err(err))
		return err
	}
...
func getFilePermissionsOrDefault(filename string, def os.FileMode) os.FileMode {
	...
	if err != nil {
		slog.Warn("Unable to get the file permissions", slog.String("filename", filename), slog.Err(err))
		return def
	}
	defer file.Close()
	...
	for {
		...
		if header.Typeflag == tar.TypeReg && header.Name == "mattermost/bin/mattermost" {
			...
			if err != nil {
				...
				if err2 != nil {
					slog.Critical("Unable to restore the backup of the executable file. Restore the executable file manually.")
			...
			if err != nil {
				slog.Warn("Unable to clean up the binary backup file.", slog.Err(err))
			}
			err = os.Chmod(executablePath, permissions)
			if err != nil {
				slog.Warn("Unable to set the correct permissions for the file.", slog.Err(err))
			}
			break
		}
	}
