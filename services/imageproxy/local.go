...
func makeLocalBackend(proxy *ImageProxy) *LocalBackend {
	...
	if proxy.Logger != nil {
		logger, err := proxy.Logger.StdLogAt(slog.LevelDebug, slog.String("image_proxy", "local"))
		if err != nil {
			slog.Warn("Failed to initialize logger for image proxy", slog.Err(err))
		}
	...
	if err != nil {
		slog.Warn("Failed to set base URL for image proxy. Relative image links may not work.", slog.Err(err))
	} else {
...
func (backend *LocalBackend) GetImage(w http.ResponseWriter, r *http.Request, imageURL string) {
	...
	if err != nil {
		...
		slog.Debug("Failed to create request for proxied image", slog.String("url", imageURL), slog.Err(err))
	...
	if err != nil {
		slog.Debug("Failed to parse URL for proxied image", slog.String("url", imageURL), slog.Err(err))
		w.WriteHeader(http.StatusBadRequest)
		w.Write([]byte{})
		return
	}
