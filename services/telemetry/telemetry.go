...
func New(srv ServerIface, dbStore store.Store, searchEngine *searchengine.Broker, log *slog.Logger) *TelemetryService {
...
func (ts *TelemetryService) ensureTelemetryID() {
	...
	if err != nil {
		slog.Error("unable to get the telemetry ID", slog.Err(err))
		return
	}
...
func (ts *TelemetryService) trackActivity() {
	...
	if err != nil {
		slog.Info("Could not get team count", slog.Err(err))
	}
...
func (ts *TelemetryService) trackGroups() {
	...
	if err != nil {
		slog.Debug("Could not get group_count", slog.Err(err))
	}

	groupTeamCount, err := ts.dbStore.Group().GroupTeamCount()
	if err != nil {
		slog.Debug("Could not get group_team_count", slog.Err(err))
	}

	groupChannelCount, err := ts.dbStore.Group().GroupChannelCount()
	if err != nil {
		slog.Debug("Could not get group_channel_count", slog.Err(err))
	}

	groupSyncedTeamCount, nErr := ts.dbStore.Team().GroupSyncedTeamCount()
	if nErr != nil {
		slog.Debug("Could not get group_synced_team_count", slog.Err(nErr))
	}

	groupSyncedChannelCount, nErr := ts.dbStore.Channel().GroupSyncedChannelCount()
	if nErr != nil {
		slog.Debug("Could not get group_synced_channel_count", slog.Err(nErr))
	}

	groupMemberCount, err := ts.dbStore.Group().GroupMemberCount()
	if err != nil {
		slog.Debug("Could not get group_member_count", slog.Err(err))
	}

	distinctGroupMemberCount, err := ts.dbStore.Group().DistinctGroupMemberCount()
	if err != nil {
		slog.Debug("Could not get distinct_group_member_count", slog.Err(err))
	}

	groupCountWithAllowReference, err := ts.dbStore.Group().GroupCountWithAllowReference()
	if err != nil {
		slog.Debug("Could not get group_count_with_allow_reference", slog.Err(err))
	}
...
func (ts *TelemetryService) trackChannelModeration() {
	...
	if err != nil {
		slog.Debug("Could not get channel_scheme_count", slog.Err(err))
	}
	...
	if err != nil {
		slog.Debug("Could not get create_post_user_disabled_count", slog.Err(err))
	}
	...
	if err != nil {
		slog.Debug("Could not get create_post_guest_disabled_count", slog.Err(err))
	}
	...
	if err != nil {
		slog.Debug("Could not get post_reactions_user_disabled_count", slog.Err(err))
	}
	...
	if err != nil {
		slog.Debug("Could not get post_reactions_guest_disabled_count", slog.Err(err))
	}
	...
	if err != nil {
		slog.Debug("Could not get manage_members_user_disabled_count", slog.Err(err))
	}
	...
	if err != nil {
		slog.Debug("Could not get use_channel_mentions_user_disabled_count", slog.Err(err))
	}
	...
	if err != nil {
		slog.Debug("Could not get use_channel_mentions_guest_disabled_count", slog.Err(err))
	}
...
func (ts *TelemetryService) initRudder(endpoint string, rudderKey string) {
	if ts.rudderClient == nil {
		...
		config.Logger = rudder.StdLogger(ts.log.StdLog(slog.String("source", "rudder")))
		...
		if err != nil {
			slog.Error("Failed to create Rudder instance", slog.Err(err))
			return
		}
...
func (ts *TelemetryService) RunTelemetryJob(firstRun int64) {
	...
	model.CreateRecurringTask("Telemetry", func() {
		ts.doTelemetryIfNeeded(util.TimeFromMillis(firstRun))
	}, time.Minute*10)
}
...
func (ts *TelemetryService) trackPluginConfig(cfg *model.Config, marketplaceURL string) {
	...
	if err != nil {
		slog.Info("Failed to fetch marketplace plugins for telemetry. Using predefined list.", slog.Err(err))
	...
	if pluginsEnvironment != nil {
		if plugins, appErr := pluginsEnvironment.Available(); appErr != nil {
			slog.Warn("Unable to add plugin versions to telemetry", slog.Err(appErr))
		} else {
